'''
    This code was AUTOGENERATED using the codama library.
    Please DO NOT EDIT THIS FILE, instead use visitors
    to add features, then rerun codama to update it.
    @see https://github.com/codama-idl/codama
'''

import borsh_construct as borsh
import typing
from solders.instruction import AccountMeta, Instruction
from solders.pubkey import Pubkey as SolPubkey
from ..program_id import PROGRAM_ID
class UpdateSpotMarketMaxTokenBorrowsArgs(typing.TypedDict):
    maxTokenBorrowsFraction:int


layout = borsh.CStruct(
    "maxTokenBorrowsFraction" /borsh.U16,
    )


class UpdateSpotMarketMaxTokenBorrowsAccounts(typing.TypedDict):
    admin:SolPubkey
    state:SolPubkey
    spotMarket:SolPubkey

def UpdateSpotMarketMaxTokenBorrows(
    args: UpdateSpotMarketMaxTokenBorrowsArgs,
    accounts: UpdateSpotMarketMaxTokenBorrowsAccounts,
    program_id: SolPubkey = PROGRAM_ID,
    remaining_accounts: typing.Optional[typing.List[AccountMeta]] = None,
) ->Instruction:
    keys: list[AccountMeta] = [
    AccountMeta(pubkey=accounts["admin"], is_signer=True, is_writable=False),
    AccountMeta(pubkey=accounts["state"], is_signer=False, is_writable=False),
    AccountMeta(pubkey=accounts["spotMarket"], is_signer=False, is_writable=True),
    ]
    if remaining_accounts is not None:
        keys += remaining_accounts
    identifier = b"\x39\x66\xcc\xd4\xfd\x5f\x0d\xc7"
    encoded_args = layout.build({
        "maxTokenBorrowsFraction":args["maxTokenBorrowsFraction"],
       })
    data = identifier + encoded_args
    return Instruction(program_id,data,keys)



